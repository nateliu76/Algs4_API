...
public class BinarySearch {
    ...
    // use pred + 1 instead after it's done
    public static int firstIndexOf(int key, int[] a) {
        if (a.length == 0) return -1;
        int lo = -1;  // setting these are for edge cases and to avoid off by 1 errors
        int hi = a.length - 1;
        while (hi - lo > 1) {
            int mid = lo + (hi - lo) / 2;
            if (Integer.compare(a[mid], key) >= 0) hi = mid;
            else lo = mid;
        }
    ...
    // use succ - 1 instead after it is done
    public static int lastIndexOf(int key, int[] a) {
        ...
        while (hi - lo > 1) {
            int mid = lo + (hi - lo) / 2;
            if (Integer.compare(a[mid], key) <= 0) lo = mid;
            else hi = mid;
        }
        if (Integer.compare(a[lo], key) == 0) return lo;
        else                                     return -1;
