import edu.princeton.cs.algs4.In;
import edu.princeton.cs.algs4.Graph;

import junit.framework.TestCase;

public class EulerianCycleTest extends TestCase {
    
    
    // To test Eulerian path and cycle, we need to check:
    // 1. if # of visits == # of edges
    // 2. if an edge exists for all the visits made between vertices
    // 3. check if correctly detected Eulerian Cycle vs Path
    // to test 1 and 2, an easy way to test it would be to make an adj matrix
    // for the undirected graph and the graph generated by following the 
    // eulerian path
    public void testCycle() {
        
        // eulerian cycle test cases
        assertTrue(checkCycle("smallCycleG.txt", true));
        for (int i = 0; i <= 6; i++) {
            String f = "cycle"+i+".txt";
            assertTrue(checkCycle(f, true));
        }
        
        // non eulerian path test case
        assertTrue(checkCycle("pathExtraEdge.txt", false));
        assertTrue(checkCycle("smallPathG.txt", false));
    }
    
    // makes graph from file and returns Eulerian path
    private boolean checkCycle(String filename, boolean hasCycle) {
        In in = new In(filename);
        Graph G = new Graph(in);
        in.close();
        
        EulerianCycle ep = new EulerianCycle(G);
        if (hasCycle != ep.hasEulerianCycle())
            return false;
        // return if this is a failing case
        if (!hasCycle) return true;
        
        // make adj matrix of G
        int[][] matrix = new int[G.V()][G.V()];
        for (int i = 0; i < G.V(); i++) {
            for (int j : G.adj(i)) {
                matrix[i][j]++;
            }
        }
        
        // deduct the edges in Eulerian path
        int prev = -1;
        for (int v : ep.cycle()) {
            if (prev == -1) {
                prev = v;
                continue;
            }
            matrix[prev][v]--;
            matrix[v][prev]--;
            prev = v;
        }
        
        // check if the edges match
        for (int i = 0; i < G.V(); i++) {
            for (int j = 0; j < G.V(); j++) {
                if (matrix[i][j] != 0) return false;
            }
        }
        
        return true;
    }
}
